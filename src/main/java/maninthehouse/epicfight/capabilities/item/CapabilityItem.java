...
public abstract class CapabilityItem {
	...
	public void modifyItemTooltip(List<String> itemTooltip, LivingData<?> entitydata) {
		...
		if(attribute != null) {
			...
			for(Entry<IAttribute, AttributeModifier> attr : attribute.entrySet()) {
	...
	public void onHeld(PlayerData<?> playerdata) {
		...
		SkillContainer skillContainerPASSIVE = playerdata.getSkill(SkillSlot.WEAPON_GIMMICK);
		
		if(skill == null) {
			skillContainerPASSIVE.setSkill(null);
		} else {
			if(skillContainerPASSIVE.getContaining() != skill) {
				skillContainerPASSIVE.setSkill(skill);
			}
		}
		
		Skill skillGuard = Skills.GUARD;
		SkillContainer skillContainerGUARD = playerdata.getSkill(SkillSlot.WEAPON_GUARD);
		if (skillGuard == null) {
			skillContainerGUARD.setSkill(null);
		} else {
			if (skillContainerGUARD.getContaining() != skillGuard/* && playerdata.getHeldItemCapability(EnumHand.MAIN_HAND) instanceof ModWeaponCapability*/) {
				skillContainerGUARD.setSkill(skillGuard);
			}
		}
	}

	public StaticAnimation getSummonMotion(PlayerData<?> playerdata) {
		return Animations.MAGIC_DASH_WAND;
	}
	public StaticAnimation getLongMagicMotion(PlayerData<?> playerdata) {return Animations.MAGIC_DASH_WAND;}
	...
	public EnumAction getUseAction(PlayerData<?> player) {
	...
	public enum WeaponCategory {
		NONE_WEAPON, AXE, FIST, GREATSWORD, HOE, PICKAXE, SHOVEL, SWORD, KATANA, SPEAR, BOW, CROSSBOW, LONGSWORD, DAGGER
	}
	...
	public enum WieldStyle {
		ONE_HAND, TWO_HAND, SHEATH, MOUNT, LIECHTENHAUER
	}
}