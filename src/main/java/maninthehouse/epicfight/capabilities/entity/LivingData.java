...
public abstract class LivingData<T extends EntityLivingBase> extends CapabilityEntity<T> {
	...
	public VisibleMatrix4f getModelMatrix(float partialTicks) {
		...
		/**if (ANCIENT_WARFARE) {
			if (this.getOriginalEntity() instanceof NpcFaction) {
				scaleX *= ((NpcFaction) this.getOriginalEntity()).getWidthModifier();
				scaleY *= ((NpcFaction) this.getOriginalEntity()).getRenderSizeModifier();
				scaleZ *= ((NpcFaction) this.getOriginalEntity()).getWidthModifier();
				if (((NpcFaction)this.orgEntity).getFaction().equals("orc")) {
					scaleY*=1.1;
				}
			}
