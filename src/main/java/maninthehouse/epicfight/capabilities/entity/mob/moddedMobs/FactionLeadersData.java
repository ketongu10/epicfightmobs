...
public class FactionLeadersData<T extends NpcFaction> extends BipedMobData<T> implements ISkillfulParry{
    private int tickSinceBlock=0;
    public FactionLeadersData() {
            super(Faction.NATURAL);
    }
    @Override
    public void onEntityJoinWorld(T entityIn) {
        super.onEntityJoinWorld(entityIn);
        notifyToReset((this.orgEntity.getWidthModifier())*0.6F, this.orgEntity.getRenderSizeModifier()*1.8F); //only for hitbox
        this.resetSize();
        this.orgEntity.getDataManager().register(DataKeys.STUN_ARMOR, Float.valueOf(7.0F*this.orgEntity.getRenderSizeModifier()));//only for hitbox
    ...
    protected void initAnimator(AnimatorClient animatorClient) {
        ...
        animatorClient.addLivingAnimation(LivingMotion.TACHI, Animations.BIPED_WALK);
    ...
    protected void animatorNPC(AnimatorClient animatorClient) {
        ...
        if (!this.orgEntity.getFaction().equals("orc")) {
            animatorClient.addLivingMixAnimation(LivingMotion.TACHI, Animations.BIPED_HOLD_LONGSWORD);
        }
        animatorClient.setCurrentLivingMotionsToDefault();
    }
    ...
    protected void playGuardAnimation() {
        ...
        if (heldItemString.equals("ancientwarfarenpc:wooden_halberd")
                ...
                || heldItemString.equals("ancientwarfarenpc:diamond_halberd")
                || heldItemString.equals("ancientwarfarenpc:death_scythe")
                || heldItemString.equals("ancientwarfarenpc:scythe") || heldItem instanceof ItemAxe) {
            guard = Animations.SPEAR_GUARD_HIT;
        } else {
            if (rnd > 0.5F) {
    ...
    public void setAIAsArmed() {
        ...
        if (this.orgEntity.getFaction().equals("orc")) {
            orgEntity.tasks.addTask(0, new EntityAIParry(this, this.orgEntity, 0.03F, 0.0D, 3.5D, true, MobAttackPatterns.ORC_SPECIAL_ATTACK1, MobAttackPatterns.ORC_SPECIAL_ATTACK3));
        } else {
    ...
    public void setAIAsHugeArmed() {
        Item offhand = this.orgEntity.getHeldItemOffhand().getItem();
        if(offhand instanceof ItemSword || offhand instanceof ItemAxe || offhand instanceof ItemExtendedReachWeapon) {
            orgEntity.tasks.addTask(0, new EntityAIAttackPattern(this, this.orgEntity, 0.0D, 3.0D, true, MobAttackPatterns.HUGE_ARMED2)); //BIPED_ARMED_2_AXES
            orgEntity.tasks.addTask(1, new EntityAIChase(this, this.orgEntity, 1.2D, false));
        } else  {
            orgEntity.tasks.addTask(0, new EntityAIAttackPattern(this, this.orgEntity, 0.0D, 3.0D, true, MobAttackPatterns.HUGE_ARMED)); //BIPED_ARMED_ONE_AXE
            orgEntity.tasks.addTask(1, new EntityAIChase(this, this.orgEntity, 1.2D, false));
        }
        if (this.orgEntity.getFaction().equals("orc")) {
            orgEntity.tasks.addTask(0, new EntityAIParry(this, this.orgEntity, 0.03F, 0.0D, 4.5D, true, MobAttackPatterns.ORC_SPECIAL_ATTACK1, MobAttackPatterns.ORC_SPECIAL_ATTACK3));
        } else {
            orgEntity.tasks.addTask(0, new EntityAIParry(this, this.orgEntity, 0.03F, 0.0D, 4.5D, true, MobAttackPatterns.PARRY_SPECIAL_ATTACK1, MobAttackPatterns.PARRY_SPECIAL_ATTACK2, MobAttackPatterns.PARRY_SPECIAL_ATTACK3));
        }
    }
