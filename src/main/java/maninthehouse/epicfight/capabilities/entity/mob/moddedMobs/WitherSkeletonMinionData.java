...
public class WitherSkeletonMinionData extends SkeletonData<EntityWitherSkeletonMinion> {
    public WitherSkeletonMinionData() {
        super(Faction.WITHER_ARMY);
    }

    @Override
    public void onEntityJoinWorld(EntityWitherSkeletonMinion entityIn) {
        super.onEntityJoinWorld(entityIn);
        this.orgEntity.getDataManager().set(DataKeys.STUN_ARMOR, 4.0F);
        if (ANCIENT_SPELLCRAFT) {
            this.orgEntity.setItemStackToSlot(EntityEquipmentSlot.MAINHAND, new ItemStack(ASItems.devoritium_sword));
        }
    }
    ...
    public boolean hurtEntity(Entity hitTarget, EnumHand handIn, IExtendedDamageSource source, float amount) {
        ...
        if (succed && hitTarget instanceof EntityLivingBase && this.orgEntity.getRNG().nextInt(1) == 0) {
            ((EntityLivingBase) hitTarget).addPotionEffect(new PotionEffect(MobEffects.WITHER, 200));
            if (ANCIENT_SPELLCRAFT) {
                ((EntityLivingBase) hitTarget).addPotionEffect(new PotionEffect(ASPotions.magical_exhaustion, 200, 2));
            }
        }
