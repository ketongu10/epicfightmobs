...
public class Skill {
	...
	public Skill(SkillSlot index, float cooldown, String skillName) {
		this(index, cooldown, 0, 1, true, skillName, 0);
	}
	
	public Skill(SkillSlot index, float cooldown, int maxStack, String skillName) {
		this(index, cooldown, 0, maxStack, true, skillName, 0);
	}
	
	public Skill(SkillSlot index, float cooldown, int duration, boolean isActiveSkill, String skillName, float chargeIn) {
		this(index, cooldown, duration, 1, true, skillName, chargeIn);
	}
	
	public Skill(SkillSlot index, float cooldown, int duration, int maxStack, boolean isActiveSkill, String skillName, float chargeIn) {
	...
	public boolean canExecute(PlayerData<?> executer) {
		return executer.getStamina() >= this.charge;//true
	}
	...
	public void update(SkillContainer container) {
		...
		container.prevCooldown = container.cooldown;//BY KETONGU10
		container.prevDuration = container.duration;
		
		if(container.stack < container.containingSkill.maxStackSize)
			container.setCooldown(container.cooldown + this.getRegenTimePerTick(executer) * GameConstants.A_TICK);//BY KETONGU10
	...
	public void decreaseStamina(ServerPlayerData executer) {
		float f1 = executer.getOriginalEntity().getDataManager().get(DataKeys.STAMINA).floatValue() - this.charge;
		executer.getOriginalEntity().getDataManager().set(DataKeys.STAMINA, f1);
	}
