...
public class SpecialAttackSkill extends Skill {
	...
	public SpecialAttackSkill(SkillSlot index, float restriction, String skillName, float chargeIn, StaticAnimation animation) {
		this(index, restriction, 0, skillName, chargeIn, animation);
	}
	
	public SpecialAttackSkill(SkillSlot index, float restriction, int duration, String skillName, float chargeIn, StaticAnimation animation) {
		super(index, restriction, duration, true, skillName, chargeIn);
	...
	public boolean canExecute(PlayerData<?> executer) {
		...
		if (item != null) {
			...
			return skill == this && executer.getOriginalEntity().getRidingEntity() == null && executer.getStamina() >= this.charge;
		}
