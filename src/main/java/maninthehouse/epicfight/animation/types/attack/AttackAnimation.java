...
import maninthehouse.epicfight.entity.event.AttackEndEvent;
...
public class AttackAnimation extends ActionAnimation {
	...
	public void onFinish(LivingData<?> entitydata, boolean isEnd) {
		...
		if (entitydata instanceof ServerPlayerData && isEnd) {
			...
			playerdata.getEventListener().activateEvents(PlayerEventListener.EventType.ATTACK_ANIMATION_END_EVENT, new AttackEndEvent(playerdata, entitydata.currentlyAttackedEntity, this.getId()));
		}
		entitydata.currentlyAttackedEntity.clear();
