...
public class CustomModelBakery {
	...
	static final ModelPart LEFT_ARM_CHILD = new SimpleSeparateBaker(16, 17, 19.0F);
	static final ModelPart RIGHT_ARM = new Limb(11, 12, 14, 19.0F, false);
	static final ModelPart RIGHT_ARM_CHILD = new SimpleSeparateBaker(11, 12, 19.0F);
	static final ModelPart LEFT_LEG = new Limb(4, 5, 6, 6.0F, true);
	static final ModelPart LEFT_LEG_CHILD = new SimpleSeparateBaker(4, 5, 6.0F);
	static final ModelPart RIGHT_LEG = new Limb(1, 2, 3, 6.0F, true);
	static final ModelPart RIGHT_LEG_CHILD = new SimpleSeparateBaker(1, 2, 6.0F);
	static final ModelPart CHEST = new Chest();
	static final ModelPart CHEST_CHILD = new SimpleSeparateBaker(8, 7, 18.0F);
	...
	public static ClientModel bakeBipedCustomArmorModel(ModelBiped model, ItemArmor armorItem) {
		...
		switch (equipmentSlot) {
			case HEAD:
				allBoxes.add(new ModelPartBind(HEAD, HEAD, model.bipedHead));
				allBoxes.add(new ModelPartBind(HEAD, HEAD, model.bipedHeadwear));
				break;
			case CHEST:
				allBoxes.add(new ModelPartBind(CHEST, CHEST_CHILD, model.bipedBody));
				allBoxes.add(new ModelPartBind(RIGHT_ARM, RIGHT_ARM_CHILD, model.bipedRightArm));
				allBoxes.add(new ModelPartBind(LEFT_ARM, LEFT_ARM_CHILD, model.bipedLeftArm));
				break;
			case LEGS:
				allBoxes.add(new ModelPartBind(CHEST, CHEST_CHILD, model.bipedBody));
				allBoxes.add(new ModelPartBind(LEFT_LEG, LEFT_LEG_CHILD, model.bipedLeftLeg));
				allBoxes.add(new ModelPartBind(RIGHT_LEG, RIGHT_LEG_CHILD, model.bipedRightLeg));
				break;
			case FEET:
				allBoxes.add(new ModelPartBind(LEFT_LEG, LEFT_FEET, model.bipedLeftLeg));
				allBoxes.add(new ModelPartBind(RIGHT_LEG, RIGHT_FEET, model.bipedRightLeg));
				break;
			default:
				return null;
		}
	...
	private static Mesh bakeMeshFromCubes(List<ModelPartBind> cubes) {
		...
		for (ModelPartBind modelPart : cubes) {
			bake(matrix4f, modelPart,modelPart.partedBaker, modelPart.modelRenderer, vertices, indices);
		}
		
		return VertexData.loadVertexInformation(vertices, ArrayUtils.toPrimitive(indices.toArray(new Integer[0])), true);
	}
	
	private static void bake(VisibleMatrix4f matrix, ModelPartBind modelpartition, ModelPart part, ModelRenderer renderer, List<VertexData> vertices, List<Integer> indices) {
		...
		if (renderer.rotateAngleZ != 0.0F) {
			internalMatrix.rotateDegree(renderer.rotateAngleZ* 180f/3.14f, new Vec3f(0, 0, 1));
		}

		if (renderer.rotateAngleY != 0.0F) {
			internalMatrix.rotateDegree(renderer.rotateAngleY* 180f/3.14f, new Vec3f(0, 1, 0));
		}

		if (renderer.rotateAngleX != 0.0F) {
			internalMatrix.rotateDegree(renderer.rotateAngleX* 180f/3.14f, new Vec3f(1, 0, 0));
		}
		...
		if (renderer.childModels != null) {
			for (ModelRenderer childRenderer : renderer.childModels) {
				bake(internalMatrix, modelpartition,part, childRenderer, vertices, indices);
			}
		}
	}
	...
	static class ModelPartBind {
		...
		ModelPart childBaker;
		
		public ModelPartBind(ModelPart partedBaker, ModelPart childBaker, ModelRenderer modelRenderer) {
	...
	@SideOnly(Side.CLIENT)
	static class SimpleSeparateBaker extends ModelPart {
		final SimplePart upperBaker;
		final SimplePart lowerBaker;
		final float yClipCoord;

		public SimpleSeparateBaker(int upperJoint, int lowerJoint, float yClipCoord) {
			this.upperBaker = new SimplePart(upperJoint);
			this.lowerBaker = new SimplePart(lowerJoint);
			this.yClipCoord = yClipCoord;
		}

		@Override
		public void bakeCube(VisibleMatrix4f poseStack, ModelBox cube, List<VertexData> vertices, List<Integer> indices) {
			Vec4f cubeCenter = new Vec4f(cube.posX1 + (cube.posX2 - cube.posX1) * 0.5F, cube.posY1 + (cube.posY2 - cube.posY1) * 0.5F, cube.posZ1 + (cube.posZ2 - cube.posZ1) * 0.5F, 1.0F);
			//cubeCenter.transform(poseStack.last().pose());
			VisibleMatrix4f.transform(poseStack, cubeCenter, cubeCenter);
	...
	static EnumFacing getDirectionFromVector(Vec3f directionVec) {
		for (EnumFacing direction : EnumFacing.values()) {
			Vec3f direcVec = new Vec3f(Float.compare(directionVec.x, -0.0F) == 0 ? 0.0F : directionVec.y, directionVec.y, directionVec.z);
