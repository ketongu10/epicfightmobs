...
public class BattleModeGui extends ModIngameGui {
	...
	public void renderGui(ClientPlayerData playerdata, float partialTicks) {
		...
		int width2 = sr2.getScaledWidth();
		int height2 = sr2.getScaledHeight();

		boolean depthTestEnabled2 = GL11.glGetBoolean(GL11.GL_DEPTH_TEST);
		boolean alphaTestEnabled2 = GL11.glGetBoolean(GL11.GL_ALPHA_TEST);
		boolean blendEnabled2 = GL11.glGetBoolean(GL11.GL_BLEND);

		if(!depthTestEnabled2)
			GlStateManager.enableDepth();
		if(!alphaTestEnabled2)
			GL11.glEnable(GL11.GL_ALPHA_TEST);
		if(!blendEnabled2)
			GlStateManager.enableBlend();
		...
		if(maxStamina > 0.0F && stamina < maxStamina) {
			float ratio2 = stamina > 0 ? stamina / maxStamina : 0;
			GlStateManager.pushMatrix();
			GlStateManager.translate(0, (float)guiSlider * 0.5F - 20.0F, 0);
			GlStateManager.scale(0.5F, 0.5F, 1.0F);
			GlStateManager.color(0.25F, ratio2, 1.0F, 1.0F);
			drawModalRectWithCustomSizedTexture((int)((width2-120) * 2.0F), (int)((height2-10) * 2.0F), 2.0F, 38.0F, 237, 9, 255, 255);
			drawModalRectWithCustomSizedTexture((int)((width2-120) * 2.0F), (int)((height2-10) * 2.0F), 2.0F, 47.0F, (int)(237*ratio2), 9, 255, 255);
			GlStateManager.popMatrix();
		}
		...
		for (int i = 0; i < SkillSlot.values().length; i++) {
			...
			if(container != null && !container.isEmpty() && this.screenPositionMap.containsKey(container.getContaining().getSlot())) {
				...
				//float cooldownRatio = creative ? 1.0F : container.getCooldownRatio(partialTicks);//BY KETONGU10
				float cooldownRatio = creative ? 1.0F : (playerdata.getStamina()>0 ? playerdata.getStamina()/playerdata.getMaxStamina() : 0);
				float durationRatio = container.getDurationRatio(partialTicks);
				boolean isReady = container.getStack() > 0 || durationRatio > 0 || creative;//BY KETONGU10
				...
				if (slot != SkillSlot.WEAPON_SPECIAL_ATTACK) {
					...
					if (!isReady) {
						GlStateManager.scale(multiplyScale, multiplyScale, 1.0F);
						//String s = String.valueOf((int)(cooldownRatio * 100.0F));//BY KETONGU10
