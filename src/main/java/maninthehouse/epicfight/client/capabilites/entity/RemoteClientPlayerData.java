...
public class RemoteClientPlayerData<T extends AbstractClientPlayer> extends PlayerData<T> {
	...
	public void updateMotion() {
		...
		} else if (!orgEntity.onGround && orgEntity.isOnLadder()) {
			currentMotion = LivingMotion.CLIMB;
			double y = orgEntity.chasingPosY - orgEntity.prevChasingPosY;
			animator.baseLayer.pause = Math.abs(y) < 0.04D;
		} else if (orgEntity.isInWater() && !orgEntity.onGround && orgEntity.moveForward > 0.0) {
			currentMotion = LivingMotion.SWIMMING;
		} else if (orgEntity.isPlayerSleeping()) {
		...
		if (this.orgEntity.isHandActive() && orgEntity.getItemInUseCount() > 0) {
			...
			ItemStack items = this.orgEntity.getHeldItem(this.orgEntity.getActiveHand());
			CapabilityItem cap = items.getCapability(ModCapabilities.CAPABILITY_ITEM, null); //by ketongu10
			if(useAction == EnumAction.BLOCK || cap instanceof SwordCapability || cap instanceof ModWeaponCapability || cap instanceof AxeCapability)
				currentMixMotion = LivingMotion.BLOCKING;
			else if(useAction == EnumAction.BOW)
				currentMixMotion = LivingMotion.AIMING;
			/**BY KETONGU10**/
			else if (EBWIZARDRY && items.getItem() instanceof ItemWand) {
				if(((ItemWand) items.getItem()).getCurrentSpell(items).isContinuous || ((ItemWand) items.getItem()).getCurrentSpell(items).getChargeup() > 0){
					if (useAction == SpellActions.SUMMON || useAction == SpellActions.POINT_UP || useAction == SpellActions.POINT_DOWN) {
						currentMixMotion = LivingMotion.SUMMONNING;
					} else {
		...
		if (this.getHeldItemCapability(EnumHand.MAIN_HAND) != null) {
			if (this.getHeldItemCapability(EnumHand.MAIN_HAND).getWeaponCategory() == CapabilityItem.WeaponCategory.LONGSWORD && !this.orgEntity.isHandActive() && (currentMotion == LivingMotion.IDLE || currentMotion == LivingMotion.WALKING)) {
