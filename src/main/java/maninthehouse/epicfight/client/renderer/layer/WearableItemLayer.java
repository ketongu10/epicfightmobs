...
import maninthehouse.epicfight.client.model.compatibility.FixedArmor;
...
public class WearableItemLayer<E extends EntityLivingBase, T extends LivingData<E>> extends Layer<E, T> {
	...
	public void renderLayer(T entitydata, E entityliving, VisibleMatrix4f[] poses, float partialTicks) {
		...
		if (item instanceof ItemArmor) {
			...
			if (VAMPIRISM) {
				if (item instanceof ItemWitcherBackpack || item instanceof ItemVampireCloak) {
		...
		if (ARMOR_MODEL_MAP.containsKey(registryName)) {
			...
			for (LayerRenderer<?> layer : entityRenderer.layerRenderers) {
				if (layer instanceof LayerArmorBase) {
					ModelBase m = ((LayerArmorBase) layer).getModelFromSlot(this.slot);
					originalModel = m instanceof ModelBiped ? (ModelBiped) ((LayerArmorBase) layer).getModelFromSlot(this.slot) : originalModel;
				}
			}

			/**==============ARMOUR RENDER FIXING BY KETONGU10================**/

			ModelBiped customModel= armorItem.getArmorModel(entityliving, stack, slot, originalModel);
			if (FixedArmor.FIXED_ARMOUR_BY_NAME.containsKey(armorItem.getRegistryName())) {
				customModel = FixedArmor.FIXED_ARMOUR_BY_NAME.get(armorItem.getRegistryName());
			} else if (EBWIZARDRY && armorItem instanceof ItemWizardArmour && slot != EntityEquipmentSlot.LEGS) {
				if (FixedArmor.FIXED_ARMOUR_BY_TYPE.containsKey(((ItemWizardArmour) armorItem).armourClass)) {
					customModel = FixedArmor.FIXED_ARMOUR_BY_TYPE.get(((ItemWizardArmour) armorItem).armourClass);
				}
			}
			/**ModelBiped customModel;
			if (FixedArmor.FIXED_ARMOUR_BY_NAME.containsKey(armorItem.getRegistryName())) {
				customModel = FixedArmor.FIXED_ARMOUR_BY_NAME.get(armorItem.getRegistryName());
			} else {
				customModel = armorItem.getArmorModel(entityliving, stack, slot, originalModel);
				if (FixedArmor.FIXED_ARMOUR_BY_CLASS.containsKey(customModel)) {
					customModel = FixedArmor.FIXED_ARMOUR_BY_CLASS.get(customModel);
				}
			}**/
			/**END**/
