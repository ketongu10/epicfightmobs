...
public class RenderItemBase {
	...
	static {
		...
		VisibleMatrix4f.translate(new Vec3f(0.5F, 1, 0.1F), BACK_COORECTION, BACK_COORECTION); //нихуя не меняет
		VisibleMatrix4f.rotate((float)Math.toRadians(130), new Vec3f(0,0,1), BACK_COORECTION, BACK_COORECTION); //z = 1
		VisibleMatrix4f.rotate((float)Math.toRadians(100), new Vec3f(0, 1, 0), BACK_COORECTION, BACK_COORECTION);  //Y= 1
	}
	
	public RenderItemBase() {
		correctionMatrix = new VisibleMatrix4f();
		VisibleMatrix4f.rotate((float)Math.toRadians(-80), new Vec3f(1.0F,0,0), correctionMatrix, correctionMatrix);  //x = 1
		VisibleMatrix4f.translate(new Vec3f(0.0F,0.1F,0.0F), correctionMatrix, correctionMatrix); //x = z =0
	}
